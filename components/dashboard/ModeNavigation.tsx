...
export function ModeNavigation({
  ...
  return (
    <nav className="grid w-full max-w-full grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      {investorModes.map((mode) => (
        <button
          key={mode.mode}
          type="button"
          onClick={() => onSelect(mode.mode)}
          className={`min-w-0 rounded-lg border px-3 py-3 text-left transition duration-200 ${
        ...
        >
          <span className="block truncate text-sm font-semibold">{mode.mode}</span>
          <span className="mt-1 block truncate text-[11px] uppercase tracking-wide text-zinc-500">{mode.shortLabel}</span>
        </button>
      ))}
    </nav>
  );
